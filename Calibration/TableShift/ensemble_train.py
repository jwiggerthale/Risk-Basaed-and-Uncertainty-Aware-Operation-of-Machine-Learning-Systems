'''
This script implements training of ensemble models
'''
...
for seed in [2, 4, 8, 13, 19 , 22, 31,  34, 44, 50, 53, 61, 68, 71, 75, 83, 86, 90, 97, 101]:
    ...
    for n_model in range(n_models):
        os.makedirs(f'./ensemble/model_{n_model}', exist_ok = True)
        criterion = nn.BCEWithLogitsLoss()
        model = heart_model(temperature = 1.0, input_dim = 142).double().to('cuda')
        model_name = f'./ensemble/model_{n_model}/model_diabetes_survey_seed_{seed}.pth'
