...
# python3 ensemble_train.py --model effnet --out_dir /data/PhDThesis/SMiLe/CIFAR/effnet --num_models 5
'''
    ...
    for epoch in range(num_epochs):
        ...
        if aug_loader is not None:
            ...
            for x, y, _ in iter(aug_loader):
                ...
                optimizer.step()
                running_loss += loss.item()
            running_loss /= (len(aug_loader) + len(train_loader))
        test_samples = 0
        model.eval()
...
# train models and save to separate folder per ensemble member
for i in range(args.num_models):
