'''
This script calculates metrics for ensemble models reported in Chapter 5 of the thesis and sotres results to json files
'''
...
import numpy as np
import os
from modules.cifar_data_utils import image_dataset, transform_test, image_dataset_c, get_cifar_files, class_map, convert_from_c_class
...
if 'resnet' in args.model.lower():
    models = [resnet18(num_classes=100) for _ in range(len(model_files))]
else:
    models = [efficientnet_b0(num_classes=100) for _ in range(len(model_files))]
...
# get predictions and calcualte metrics per dataset
preds = []
labels = []
eus = []
sigmas = []
