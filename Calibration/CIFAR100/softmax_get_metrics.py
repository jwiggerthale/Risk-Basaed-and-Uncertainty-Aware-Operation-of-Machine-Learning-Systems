...
import numpy as np
import os
from modules.cifar_data_utils import image_dataset, transform_test, image_dataset_c, get_cifar_files, class_map, convert_to_c_class, convert_from_c_class
...
parser.add_argument(
    "--save_dir",
    default="./mc/balanced_resnet18",
    help="Parent dir where dirs for each ensemble are loacted",
)
...
# get predictions and metrics per dataset
preds = []
labels = []
eus = []
sigmas = []
...
# get rersults for corrupted data by severity
c_preds = []
c_labels = []
c_pred_cls = []
c_uncs = []
    ...
    for key, dl in c_sets.items():
        if f'sev_{severity}' in key:
            ...
            for x, y in iter(dl):
                ...
                c_preds.extend(torch.softmax(mu, dim=-1).detach().cpu().tolist())
                entropy = -(mu.clamp_min(1e-8) * mu.clamp_min(1e-8).log()).sum(dim=1)
                pred = mu.argmax(dim = 1)
                pred_cls.extend(pred.cpu().detach().tolist())
            ...
            # uncomment this code if you also need results per augmentation type
            '''
...
# evaluate results for all severities combined
model_stats = {}
...
aurc_eu = 0
aurc_au = 0
nll = compute_nll_multiclass(y_true = np.array(c_labels), probs = np.array(c_preds))
