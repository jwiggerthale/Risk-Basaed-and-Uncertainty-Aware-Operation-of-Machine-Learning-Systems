...
'''
This function implements calculation of ece
call with:
...
use when n is large
'''
def median_heuristic_bandwidth(P: np.ndarray, max_pairs: int = 200_000, rng: int = 0) -> float:
    n = P.shape[0]
    if n < 2:
        return 1.0
...
# methods b, uq and ul are implemented
def skce_estimators_scalarI(
    ...
    # get nu 
    if nu is None:
        nu = median_heuristic_bandwidth(probs, rng=rng)  # used in paper experiments  [oai_citation:3‡Calibration Tests Multi Class.pdf](sediment://file_00000000e2487243be4e15e22fa439b9)

    R = one_hot(labels, m) - probs  # residuals r_i = e_{y_i} - p_i

    est = estimator.lower()
    if est == "ul":
        # linear-time unbiased estimator: average over pairs (1,2), (3,4), ...  [oai_citation:4‡Calibration Tests Multi Class.pdf](sediment://file_00000000e2487243be4e15e22fa439b9)
        t = n // 2
        if t == 0:
            skce = 0.0
        else:
    ...
    if block_size is None:
        ...
        if est == "b":
            skce = float(np.mean(H))  # (1/n^2) sum_{i,j} h_ij  [oai_citation:5‡Calibration Tests Multi Class.pdf](sediment://file_00000000e2487243be4e15e22fa439b9)
        elif est == "uq":
            s = np.triu(H, k=1).sum()
            skce = float(2.0 * s / (n * (n - 1)))  # 2/(n(n-1)) sum_{i<j} h_ij  [oai_citation:6‡Calibration Tests Multi Class.pdf](sediment://file_00000000e2487243be4e15e22fa439b9)
        else:
...
'''
def p_value_skce_ul(probs: np.ndarray, labels: np.ndarray, nu: float | None = None) -> dict[str, float]:
    ...
    # Lemma 3 implies sqrt(t) * SKCE_ul approx Normal(0, s_hat) under H0  [oai_citation:11‡Calibration Tests Multi Class.pdf](sediment://file_00000000e2487243be4e15e22fa439b9)
    if s_hat <= 0:
...
def plot_multiclass_calibration(probs: list, labels:list, 
    ...
    # create subplot for each class and one combined 
...
def plot_aggregated_calibration(probs, 
    ...
    ax.plot([0, 1], [0, 1], 'k--', label='Perfekt calibration')
...
def plot_abstained_prediction(results: dict, 
    ...
    plt.xlabel('Share of Samples Classified', fontsize = 20)
...
def compute_aurc(y_true, 
    ...
    y_true: array-like, shape (N,)
    y_pred: array-like, shape (N,)
    uncertainty: array-like, shape (N,)
