...
if 'resnet' in args.model.lower():
    models = [resnet18(num_classes=5) for _ in range(len(model_files))]
else:
    models = [efficientnet_b0(num_classes=5) for _ in range(len(model_files))]
...
confs = np.array(preds).max(axis = 1)
uncertainties = np.array(sigmas).max(axis = 1) + np.array(eus)
...
uncertainties = np.array(all_sigmas).max(axis = 1) + np.array(all_eus)
...
aurc_au, _, _ = compute_aurc(y_true = all_labels, 
                    y_pred = all_pred_cls, 
                    uncertainty=np.array(all_sigmas).max(axis = 1))
