...
cost_mat_scaled = cost_mat * [[22], [4], [1], [21], [3]]
...
for s in ['test', 'sev_1', 'sev_2', 'sev_3', 'sev_4', 'sev_5']:
    c_dir = f'./data/Severstal/res_en/{s}'
    ...
    model_type = 'rn'
    c_dir = f'./data/Severstal/res_rn/{s}'
    ...
    pred_cls_test_rn, all_epr_test_rn = get_optimal_decision(preds = preds_test_rn, 
                                    cost_mat = cost_mat)
    #pred_cls_test = preds_test.argmax(axis = 1)
...
with open('./data/Severstal/results.json', 'w', encoding = 'utf-8') as out_file:
